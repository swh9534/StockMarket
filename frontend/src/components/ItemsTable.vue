...
const select = (item: any) => {
  ...
  emit("row-click", item); // 'row-click' 이벤트 추가
};
...
<template>
  <div class="m-0 p-0" :class="tableClass">
    <table
      class="table table-hover border-top border-secondary-subtle table-bordered table-sm mb-0"
    >
      <thead v-if="!props.noheader">
        <tr>
          ...
          <th v-if="!props.nosetting" :class="headerClass">
            <i
              v-if="hasToggled('th')"
              class="btn bi bi-dash-circle-fill p-0"
              @click.prevent="addNew"
            ></i>
            <i
              v-else
              class="btn bi bi-plus-circle-fill p-0"
              :class="{
                'text-black-50': props.disableSetting || props.noaddition,
              }"
              @click.prevent="addNew"
            ></i>
          </th>
        ...
        <template v-for="(item, index) in props.items" :key="index">
          <tr>
            <td
              ...
              class="text-truncate"
              :class="[
                align(item[header.value]),
                props.small ? 'form-control-sm' : '',
              ]"
            ...
            >
              ...
              <template v-else-if="isObject(item[header.value])">
                <select
                  ...
                  :class="getColor(item[header.value].value)"
                  v-model="item[header.value].value"
                  :id="elementId"
                  :disabled="props.disabled"
                >
                  <option
                    v-for="(option, index) in item[header.value].options"
                    :class="getColor(option.value)"
                    :key="elementId + '-' + index"
                    :value="option.value"
                  >
                    {{ option.label }}
            ...
            <td
              v-if="!props.nosetting"
              class="text-center"
              @click.prevent="setting(index, item)"
            >
              <i
                v-if="hasToggled(index)"
                class="btn bi bi-dash-circle p-0 border-0"
              ></i>
